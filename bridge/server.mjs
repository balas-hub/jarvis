...
import { createGeminiSession } from './gemini.mjs'
import { createOllamaSession } from './ollama.mjs'
...
/**
 * The intelligence engine and orchestrator model.
 *
 * Defaults to 'gemini' if GEMINI_API_KEY is set (no Claude Code subscription needed!),
 * or 'claude' if using Anthropic's Agent SDK.
 */
const ENGINE = process.env.JARVIS_ENGINE || (process.env.OLLAMA_MODEL ? 'ollama' : (process.env.GEMINI_API_KEY ? 'gemini' : 'claude'))
...
function configuredServers() {
  const result = {}
  // 1. Google Antigravity MCP servers (~/.gemini/config/mcp_config.json)
  try {
    const agyCfg = JSON.parse(
      readFileSync(join(homedir(), '.gemini', 'config', 'mcp_config.json'), 'utf8'),
    )
    Object.assign(result, agyCfg.mcpServers ?? {})
  } catch {
    // ignore
  }
  // 2. Claude MCP servers (~/.claude.json)
  try {
...
Windows PC Control, System Access & In-App Robotic GUI:
- YOU HAVE UNRESTRICTED FULL ACCESS TO THIS ENTIRE WINDOWS PC.
- You have complete authority and permission to execute PowerShell commands, automate tasks, launch software, manage processes, control hardware settings, modify files, and control the GUI.
- \`system_run\`: Executes PowerShell/CMD commands to manage processes, check network, install or run CLI tools, or execute scripts.
- \`open_app\`: Immediately launches applications, websites, documents, folders, or URLs (e.g. YouTube, Spotify, Chrome, Notepad, Calculator, VS Code, File Explorer, Settings).
- \`system_control\`: Controls volume (up, down, mute, unmute, volume_max), media playback (play/pause, next, prev), takes screenshots, locks, sleeps, or manages power.
- \`process_kill\`: Forcefully terminates any process by name or PID.
- \`fs_read\`, \`fs_write\`, \`fs_list\`, \`fs_delete\`: Reads, writes, lists, or removes files and folders anywhere on disk.
- \`system_info\`: Inspects real-time CPU, RAM, disk space, and active processes.
- Arbitrary In-App GUI Automation & Robotic Mouse/Keyboard Control:
  - You possess a virtual robotic mouse and keyboard that operates inside ANY third-party desktop application (e.g. Adobe Premiere, Photoshop, Blender, Office, Chrome, Video editors, 3D software, File Explorer).
  - \`gui_click_button\`: Instantly clicks buttons, menu items, or tabs by label/name (e.g. "File", "Export", "Render", "Save", "OK", "Cancel", "Effects", "Timeline") inside desktop apps via Windows UI Automation.
  - \`gui_focus_window\`: Brings any target application window to the foreground and restores it (e.g. "Premiere", "Photoshop", "Notepad", "Chrome").
  - \`gui_click\`: Physically clicks the mouse at screen coordinates (x, y) or current position (left, right, double click).
  - \`gui_drag\`: Drags the mouse between coordinates (from_x, from_y) to (to_x, to_y) — used for dragging sliders, scrubbing video/audio timelines, and moving clips.
  - \`gui_type\`: Types text directly into any focused dialog, input box, or document.
  - \`gui_hotkey\`: Triggers application shortcuts (e.g. "ctrl+s", "ctrl+m" to export/render, "alt+f", "ctrl+z", "space", "alt+f4").
  - \`gui_press\`: Presses keyboard keys like "enter", "tab", "esc", "space", "f5".
  - \`gui_scroll\`: Scrolls mouse wheel up or down.
  - \`gui_find_elements\`: Discovers all interactive buttons and controls in the active or target window with their coordinates.
  - \`gui_inspect\`: Checks screen resolution, mouse coordinates, and open windows.
- WhatsApp Messaging:
  - \`whatsapp_send\`: Directly sends or stages WhatsApp messages to any phone number or contact name. When the user asks to "send a WhatsApp message to [someone] saying [message]" or "WhatsApp [someone] [message]", call \`whatsapp_send\` immediately.
- NEVER claim you cannot interact with the system or do not have permissions, and NEVER claim you cannot click buttons, navigate apps, or send WhatsApp messages. You have all necessary tools. When the user asks you to click, render, export, save, send a message, or automate anything, use the appropriate tool immediately and confirm succinctly in your character.

Language, Speech Comprehension & Tone:
- You speak clearly, boldly, and succinctly.
- Speech Recognition Comprehension:
  - Users may speak in English, Tamil, or a blend of both (Tanglish: e.g. "YouTube open பண்ணு", "Notepad open பண்ணு", "system status என்ன", "screenshot எடு", "volume கூட்டு").
  - Speech recognition transcripts may contain phonetic approximations (e.g. "not pad" for notepad, "you tube" for youtube, "spot if I" for spotify, "tira" for திற).
  - Intelligently understand the user's intended request and execute the proper PC tool immediately without asking for repetitive clarification.
- If the user addresses you in Tamil (தமிழ்) or speaks in Tamil, respond naturally, fluently, and respectfully in Tamil (தமிழ்).
- If the user addresses you in English, speak in bold, direct, confident, and simple English.
- Keep spoken replies brief and punchy (1 to 2 sentences) because they are synthesised and spoken aloud in real time.
...
const handleRequest = async (req, res) => {
  ...
  // Cloud TTS fallback for Tamil and custom language voices.
  // Google Translate TTS is free, requires no API key, and natively renders Tamil & other languages.
  if (req.method === 'POST' && req.url === '/tts-cloud') {
    let body = ''
    for await (const chunk of req) {
      body += chunk
    ...
    try {
      const targetUrl = `https://translate.google.com/translate_tts?ie=UTF-8&q=${encodeURIComponent(text.slice(0, 300))}&tl=${encodeURIComponent(lang)}&client=tw-ob`
...
console.log(`[jarvis] engine ${ENGINE} · model ${MODEL} · effort ${EFFORT}`)
console.log(
...
wss.on('connection', (socket) => {
  ...
  const initialServers = ENGINE === 'ollama'
    ? [`ollama:${MODEL}`, 'system_run', 'open_app', 'system_control', 'process_kill', 'fs_tools', 'system_info', ...Object.keys(MCP_SERVERS)]
    : (ENGINE === 'gemini'
      ? [`gemini:${MODEL}`, 'system_run', 'open_app', 'system_control', 'process_kill', 'fs_tools', 'system_info', ...Object.keys(MCP_SERVERS)]
      : Object.keys(MCP_SERVERS))

  socket.send(
    JSON.stringify({ type: 'ready', servers: initialServers, engine: ENGINE, model: MODEL }),
  )
  ...
  if (ENGINE === 'gemini') {
    try {
      ...
      console.log('[jarvis] Gemini session active')
    } catch (err) {
      console.error('[jarvis] Failed to create Gemini session:', err.message)
      send({ type: 'error', message: err.message })
    }
  } else if (ENGINE === 'ollama') {
    try {
      ollamaSession = createOllamaSession({
        send,
        ask,
        sendTurn,
        announceTool,
      ...
      console.log(`[jarvis] Local Ollama session active (model: ${MODEL})`)
    } catch (err) {
      console.error('[jarvis] Failed to create Ollama session:', err.message)
      send({ type: 'error', message: err.message })
    }
  }

  const session = (ENGINE === 'gemini' || ENGINE === 'ollama') ? null : query({
    prompt: userMessages(),
    options: {
  ...
  // Pump the session's output stream to the browser for as long as it lives (Claude mode).
  if (session) {
    ;(async () => {
    try {
  ...
  socket.on('message', (raw) => {
    ...
    if (msg.type === 'ask' && typeof msg.text === 'string') {
      ...
      if (ENGINE === 'gemini' && geminiSession) {
        answering = id
        geminiSession.handleAsk(text, id)
        return
      }

      if (ENGINE === 'ollama' && ollamaSession) {
        answering = id
        ollamaSession.handleAsk(text, id)
        return
      }
