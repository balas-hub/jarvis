...
import { getActiveProfile } from './tts'
...
const pickWorkingEn = makePicker(WORKING_EN)
const pickWorkingTa = makePicker(WORKING_TA)
const pickAckEn = makePicker(ACKNOWLEDGE_EN)
const pickAckTa = makePicker(ACKNOWLEDGE_TA)
const pickAttEn = makePicker(ATTENTION_EN)
const pickAttTa = makePicker(ATTENTION_TA)

export function working(): string {
  try {
    const p = getActiveProfile()
    if (p.lang.startsWith('ta')) return pickWorkingTa()
  } catch {}
  return pickWorkingEn()
}

export function acknowledge(): string {
  try {
    const p = getActiveProfile()
    if (p.lang.startsWith('ta')) return pickAckTa()
  } catch {}
  return pickAckEn()
}

export function attention(): string {
  try {
    const p = getActiveProfile()
    if (p.lang.startsWith('ta')) return pickAttTa()
  } catch {}
  return pickAttEn()
}
...
export function forTool(toolName: string): string {
  try {
    const p = getActiveProfile()
    if (p.lang.startsWith('ta')) {
      const { server, tool } = split(toolName)
      const combined = `${server} ${tool}`
      if (/playwright|puppeteer|browserbase|chrome|\bbrowser\b|navigate/.test(combined)) {
        return 'உலாவியைத் திறக்கிறேன்.'
      }
      if (/search|\bweb\b|\bfetch\b|crawl|research/.test(combined)) {
        return 'இணையத்தில் தேடுகிறேன்.'
      }
      if (/system|powershell|run|exec|command|cmd/.test(combined)) {
        return 'கட்டளையை இயக்குகிறேன்.'
      }
      if (/fs_|file|read|write/.test(combined)) {
        return 'கோப்பைப் படிக்கிறேன்.'
      }
      return working()
    }
  } catch {}
