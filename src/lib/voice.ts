...
export function setSttLang(lang: string): void {
  if (typeof localStorage !== 'undefined') {
    localStorage.setItem(STT_LANG_KEY, lang)
  }
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('jarvis:stt_lang_changed', { detail: lang }))
  }
}
 ...
 */
const OVERRIDE =
  /(?:\b(stop|wait|jarvis|cancel|enough|quiet|hold on|shut up|never ?mind|forget it|no|open|play|check|run|start|hello|hi|yes)\b|(?:நிறுத்து|பொறு|போதும்|ஜார்விஸ்|திற|போடு|செய்|வணக்கம்))/i
...
function isEcho(heard: string, spoken: string): boolean {
  ...
  const mine = new Set(norm(spoken).split(' ').filter(Boolean))
  ...
  return hits / content.length >= 0.7
}
