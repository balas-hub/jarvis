...
export const VOICE_PROFILES: VoiceProfile[] = [
  {
    id: 'bold_male_tamil',
    name: 'Bold Male Tamil',
    label: 'Bold Male Tamil (கம்பீர ஆண் குரல்)',
    sublabel: 'ஆழ்ந்த கம்பீரமான தமிழ் குரல்',
    lang: 'ta-IN',
    gender: 'male',
    pitch: 0.85,
    rate: 0.92,
    sample: 'வணக்கம். நான் ஜார்விஸ். தங்களுக்கான கட்டளைக்கு காத்திருக்கிறேன்.',
  },
  {
    id: 'female_tamil',
    name: 'Female Tamil',
    label: 'Female Tamil (தெளிவான பெண் குரல்)',
    sublabel: 'தெளிவான, நேர்த்தியான தமிழ் குரல்',
    lang: 'ta-IN',
    gender: 'female',
    pitch: 1.10,
    rate: 0.96,
    sample: 'வணக்கம். நான் ஜார்விஸ். கணினி அமைப்புகள் அனைத்தும் தயார் நிலையில் உள்ளன.',
  },
  {
    id: 'bold_male_english',
    name: 'Bold Male English',
    label: 'Bold Male English (Deep & Simple)',
    sublabel: 'Deep, confident baritone with direct phrasing',
    lang: 'en-US',
    gender: 'male',
    pitch: 0.85,
    rate: 0.94,
    sample: 'Systems are fully operational, sir. Standing by for your command.',
  },
  {
    id: 'classic_jarvis',
    name: 'Classic JARVIS',
    label: 'Classic JARVIS (British Butler)',
    sublabel: 'Original British AI assistant cadence',
    lang: 'en-GB',
    gender: 'male',
    pitch: 0.95,
    rate: 0.92,
    sample: 'At your service, sir. All protocols are online and functioning.',
  },
]

const VOICE_PREF_KEY = 'jarvis.voice'
const PROFILE_PREF_KEY = 'jarvis.voice_profile_id'

export function getActiveProfile(): VoiceProfile {
  if (typeof localStorage === 'undefined') return VOICE_PROFILES[2] // default to bold_male_english
  const saved = localStorage.getItem(PROFILE_PREF_KEY)
  const found = VOICE_PROFILES.find((p) => p.id === saved)
  return found ?? VOICE_PROFILES[2] // default: Bold Male English
}

export function setActiveProfile(id: VoiceProfileId): VoiceProfile {
  const p = VOICE_PROFILES.find((x) => x.id === id) ?? VOICE_PROFILES[2]
  if (typeof localStorage !== 'undefined') {
    localStorage.setItem(PROFILE_PREF_KEY, p.id)
  }
  cachedVoice = undefined
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('jarvis:voice_changed', { detail: p }))
  }
  return p
}
...
/** Best-first list of usable voices. */
...
export function pickVoice(): SpeechSynthesisVoice | null {
  ...
  // 1. If Tamil profile, search for installed Tamil voices
  if (profile.lang.startsWith('ta')) {
    const tamilVoices = all.filter((v) => /ta[-_]in|^ta/i.test(v.lang) || /tamil|valluvar|pallavi/i.test(v.name))
    if (profile.gender === 'female') {
      const female = tamilVoices.find((v) => /pallavi|female|woman/i.test(v.name))
      if (female) return (cachedVoice = female)
    } else {
      const male = tamilVoices.find((v) => /valluvar|male|man/i.test(v.name))
      if (male) return (cachedVoice = male)
    }
    if (tamilVoices.length > 0) return (cachedVoice = tamilVoices[0])
  }

  // 2. If Bold Male English, prioritize deep male English voices
  if (profile.id === 'bold_male_english') {
    const maleEn = all.find((v) =>
      /^en/i.test(v.lang) &&
      /david|mark|richard|george|google uk english male|daniel|oliver|guy/i.test(v.name)
    )
    if (maleEn) return (cachedVoice = maleEn)
  }

  // 3. User manual override if saved
...
export function cycleVoice(): string {
  ...
  const idx = VOICE_PROFILES.findIndex((p) => p.id === current.id)
  const next = VOICE_PROFILES[(idx + 1) % VOICE_PROFILES.length]
...
export function createSpeaker(): Speaker {
  ...
  function synthesise(text: string): Promise<string | null> | null {
    ...
    const isTamilProfile = profile.lang.startsWith('ta')

    // If Tamil voice or text is used and no local Tamil voice is available,
    // generate natural Tamil speech from the bridge's cloud TTS endpoint
    if (isTamilText || isTamilProfile) {
      const localVoice = pickVoice()
      const hasLocalTamil = localVoice && (/ta[-_]in|^ta/i.test(localVoice.lang) || /tamil/i.test(localVoice.name))
      if (!hasLocalTamil) {
        diag.engine = 'system'
        return fetchCloudTamilAudio(text, profile).catch(() => null)
      }
    }
  ...
  const speakNative = (text: string) =>
    new Promise<boolean>((resolve) => {
      ...
      if (!nativeInFlight && speechSynthesis.speaking) {
        try {
  ...
  const playUrl = (url: string, text: string) =>
    new Promise<void>((resolve) => {
      ...
      const profile = getActiveProfile()
      if (profile.id === 'bold_male_tamil') {
        audio.playbackRate = 0.94
      } else if (profile.id === 'female_tamil') {
...
/**
 * Fetches synthesized Tamil audio from the bridge's cloud TTS endpoint.
 */
export async function fetchCloudTamilAudio(text: string, profile: VoiceProfile): Promise<string | null> {
  try {
    const isTamilText = /[\u0B80-\u0BFF]/.test(text)
    const lang = isTamilText ? 'ta' : 'en'
    const res = await fetch(`${BRIDGE_HTTP_URL}/tts-cloud`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ text, lang, gender: profile.gender }),
    })
    if (res.ok) return URL.createObjectURL(await res.blob())
  } catch (err) {
    console.warn('[jarvis] tts-cloud error:', err)
  }
  return null
}

/**
 * Audition / test a voice profile with a preview utterance.
 */
export async function testVoice(profileId: VoiceProfileId): Promise<void> {
  const profile = VOICE_PROFILES.find((p) => p.id === profileId) ?? VOICE_PROFILES[0]
  const sample = profile.sample

  const isTamil = profile.lang.startsWith('ta')
  const all = typeof speechSynthesis !== 'undefined' ? speechSynthesis.getVoices() : []
  const hasLocalTamil = all.some((v) => /ta[-_]in|^ta/i.test(v.lang) || /tamil/i.test(v.name))

  if (isTamil && !hasLocalTamil) {
    const url = await fetchCloudTamilAudio(sample, profile)
    if (url) {
      const audio = new Audio(url)
      if (profile.id === 'bold_male_tamil') {
        audio.playbackRate = 0.94
  ...
  if (isTamil) {
    const tamilVoices = all.filter((v) => /ta[-_]in|^ta/i.test(v.lang) || /tamil|valluvar|pallavi/i.test(v.name))
    matchedVoice = profile.gender === 'female'
      ? tamilVoices.find((v) => /pallavi|female/i.test(v.name)) ?? tamilVoices[0]
      : tamilVoices.find((v) => /valluvar|male/i.test(v.name)) ?? tamilVoices[0]
  } else if (profile.id === 'bold_male_english') {
    matchedVoice = all.find((v) => /^en/i.test(v.lang) && /david|mark|richard|george|male/i.test(v.name)) ?? null
  } else if (profile.id === 'classic_jarvis') {
    matchedVoice = all.find((v) => /^en/i.test(v.lang) && /daniel|google uk english male|oliver/i.test(v.name)) ?? null
  }

  if (matchedVoice) u.voice = matchedVoice
  u.lang = matchedVoice?.lang ?? profile.lang
  u.rate = profile.rate
  u.pitch = profile.pitch
  speechSynthesis.speak(u)
}
