...
/** The same mishearings and Tamil variants voice.ts accepts for the wake word */
const EN_NAME = '(?:jarvis|jarvys|jervis|travis|jarviss|java\'s|jarv)'
const TA_NAME = '(?:ஜார்விஸ்|சார்விஸ்|ஜார்வீஸ்|ஜார்விச|ஜார்வி)'
const EN_PREFIX = '(?:hey|hi|hello|ok|okay|yo)'
const TA_PREFIX = '(?:ஹே|ஹேய்|ஏய்|வணக்கம்|ஹலோ|சொல்லு)'

/** A bare vocative — "Jarvis", "hey jarvis", "ஜார்விஸ்", "ஹே ஜார்விஸ்" — with nothing asked. */
const BARE_NAME = new RegExp(
  `^(?:(?:${EN_PREFIX}\\s+)?${EN_NAME}|(?:${TA_PREFIX}\\s+)?${TA_NAME})[\\s,.!?]*$`,
  'i'
)
/** A leading vocative on a real command: "Jarvis, what's the weather", "ஜார்விஸ் இன்றைய நேரம் என்ன". */
const LEADING_NAME = new RegExp(
  `^(?:(?:${EN_PREFIX}\\s+)?${EN_NAME}\\b|(?:${TA_PREFIX}\\s+)?${TA_NAME})[\\s,.:!?-]*`,
  'i'
)
  ...
  const respond = async (said: string): Promise<void> => {
    ...
    try {
      ...
      if (msg.includes('RESOURCE_EXHAUSTED') || msg.includes('429')) {
        msg = 'API rate limit reached. Retrying with backup model...'
    ...
    watchUi((op, args) => {
      ...
      switch (op) {
        ...
        case 'screen':
          ...
          break
        case 'gestures': {
          const gArgs = args as { enabled?: boolean }
          if (gArgs?.enabled) {
            void hands
              .enableHands()
              ...
              .catch((err: Error) => {
                s.setGestures(false)
                s.setError(`Gesture control failed: ${err.message}`)
              })
          } else {
            hands.disableHands()
            s.setGestures(false)
  ...
  useEffect(() => {
    const handleCommand = (e: Event) => {
      ...
      if (text && typeof text === 'string') {
        ...
        if (ph === 'offline') {
          void powerOn().then(() => {
            setTimeout(() => void respond(text), 600)
          })
        } else {
          void respond(text)
        }
