...
function Rig() {
  ...
  useFrame((state, dt) => {
    ...
    // Camera drift with smooth 3D parallax tracking when hand gestures are active
    const t = state.clock.elapsedTime
    let targetCamX = Math.sin(t * 0.13) * 0.35
    let targetCamY = Math.cos(t * 0.17) * 0.22

    if (hands.length > 0) {
      const h = hands[0]
      const winW = typeof window !== 'undefined' ? window.innerWidth : 1920
      const winH = typeof window !== 'undefined' ? window.innerHeight : 1080
      const nx = -(h.x / winW - 0.5) * 2
      const ny = -(h.y / winH - 0.5) * 2
      targetCamX = nx * 1.5
      targetCamY = ny * 1.1

      // Reactor subtly flares when pinching
      if (h.pinched) {
    ...
    state.camera.position.y += (targetCamY - state.camera.position.y) * Math.min(1, dt * 5)
